...
public class UniCache 
{
	...
	public static bool SwitchToBuildTarget(BuildTarget new_build_target)
	{
		...
		if(!CacheCurrentBuildTarget(unity_metadata_root))
		{
			...
			Debug.LogError("Couldn't cache the current target");
			return false;
		}
	...
	private static bool CacheCurrentBuildTarget(string unity_metadata_root)
	{
		...
		{
			string asset_relative_path = project_asset.Substring(Application.dataPath.Length-6); // up to "assets/"
			...
			if(platform_cached_assets.ContainsKey(relative_guid_filepath))
			{
				...
				{
					//Debug.Log("Overwriting cached file: " + asset_relative_path);
					File.Copy(source_cache_filepath, dest_cache_filepath, true);
				}
				else
				{
					//Debug.Log("File is up to date in cache: " + asset_relative_path);
				}
			}
			else
			{
				//Debug.Log(string.Format("Copying new cache entry: {0} to: {1}", source_cache_filepath, dest_cache_filepath));
	...
	private static void FillBuildTargetFromCache(string unity_metadata_root, BuildTarget build_target)
	{
		...
		{
			//Debug.Log("Handling asset: " + cached_asset.Value.Name);
			...
			if(asset_path == "")
			{
				...
				//Debug.Log("Deleting old cached file: " + cached_asset.Key);
			...
			else
			{
				...
				if(File.GetLastWriteTimeUtc(real_asset_path) <= cached_asset.Value.LastWriteTimeUtc)
				{
					...
					//Debug.Log(string.Format("Copying from {0} to {1}", cached_asset.Value.FullName, cache_destination));
